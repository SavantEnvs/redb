...
LOG="$(mktemp)"
# Match build.sh's CXXFLAGS so cargo's fingerprint doesn't force a recompile here.
CXXFLAGS="${CXXFLAGS:-} -include cstdint" env -u RUSTFLAGS cargo test -p "$REDB_PKG" --lib --tests 2>&1 | tee "$LOG"
run_rc="${PIPESTATUS[0]}"
