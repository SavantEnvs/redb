...
# `--tests` unifies dev-dependencies across the whole package, so bench-only C++ deps
# (librocksdb-sys 0.10.0, bundled with an old rocksdb whose util/string_util.h uses
# uint*_t without including <cstdint>) get built too. clang's libc++ no longer pulls
# <cstdint> in transitively via other standard headers, so force it.
CXXFLAGS="${CXXFLAGS:-} -include cstdint" env -u RUSTFLAGS cargo test --no-run -p "$REDB_PKG" --lib --tests
